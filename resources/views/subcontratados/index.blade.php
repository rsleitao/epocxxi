<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if (auth()->user()->hasPermission('subcontratados.create'))
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 border-b border-gray-200">
                    <form method="get" action="{{ route('subcontratados.index') }}" class="flex gap-2 flex-wrap items-center">
                            ...
                            @forelse ($subcontratados as $subcontratado)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-right space-x-2">
                                        @if (auth()->user()->hasPermission('subcontratados.edit'))
                                            <a href="{{ route('subcontratados.edit', $subcontratado) }}" class="text-epoc-primary hover:text-epoc-primary-hover">Editar</a>
                                        @endif
                                        @if (auth()->user()->hasPermission('subcontratados.delete'))
                                            <form action="{{ route('subcontratados.destroy', $subcontratado) }}" method="post" class="inline" onsubmit="return confirm('{{ $subcontratado->ativo ? 'Desativar' : 'Ativar' }} este subcontratado?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="{{ $subcontratado->ativo ? 'text-red-600 hover:text-red-900' : 'text-emerald-600 hover:text-emerald-800' }}">
                                                    {{ $subcontratado->ativo ? 'Desativar' : 'Ativar' }}
                                                </button>
                                            </form>
                                        @endif
                                    </td>
